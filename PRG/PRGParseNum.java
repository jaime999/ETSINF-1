...
public class PRGParseNum {
    ...
    public static int parseUnsignedInt(String s) {
        ...
        // Caso general: String no vacía. Tratar substring anterior.
            ...
            char c = s.charAt(len - 1);
            if (c >= '0' && c <= '9') {
                return (c - '0') + parseUnsignedInt(s.substring(0, len - 1))*10;
            } 
            else { return parseUnsignedInt(s.substring(0, len - 1)); }
        }
    }  
    ...
    public static int parseInt(String s) {
        if (s.length() == 0) { return 0; }
           ...
           //Caso general: String no vacía. Tratar substring anterior.
            int len = s.length();
            char c = s.charAt(len - 1);
            if(s.charAt(0)=='-') {return parseUnsignedInt(s.substring(0, len ))*-1;}
            else {return parseUnsignedInt(s.substring(0, len ));}
        }
    }    
    ...
    public static int posFracSep(String s) {
        ...
        // Caso general: String no vacía. Tratar substring anterior.
         ...
         if (c == PUNT || c== COMA ) {
          ...
          else { return posFracSep(s.substring(0, len - 1)); }
        }
    }
    ...
    public static double parseFrac(String s) {
         if (s.length() == 0) { return 0; }
           ...
           //Caso general: String no vacía. Tratar substring anterior.
           int len = s.length();
           return parseUnsignedInt(s)*Math.pow(10,-len);
    ...
    public static double parseDouble(String s)   {
        ...
        double decimal=0;
         if ( posFracSep(s) > 0){
          entera = parseInt(s.substring(0,posFracSep(s))); }
            
             
         if(posFracSep(s) !=-1){
             decimal = parseFrac(s.substring(posFracSep(s),s.length())); }
             
         if (entera > 0){return entera + decimal;}
         else {return entera -decimal;}
    }
